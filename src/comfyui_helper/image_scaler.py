...
class ImageScaler:
    ...
    def batch_scale(self,
        ...
        Args:
            input_dir: 输入目录
            output_dir: 输出目录，如果为None则在输入目录创建scaled子目录
            scale_factor: 缩放倍数
            target_width: 目标宽度
            target_height: 目标高度
            keep_aspect_ratio: 是否保持宽高比
        ...
        try:
            ...
            if output_dir is None:
                # 在输入目录的同级别创建带 _scaled 后缀的目录
                parent_dir = input_path.parent
                dir_name = input_path.name
                output_path = parent_dir / f"{dir_name}_scaled"
            else:
                output_path = Path(output_dir)
            output_path.mkdir(parents=True, exist_ok=True)
            batch_result["output_dir"] = str(output_path)
            ...
            if not image_files:
                batch_result["message"] = "未找到符合条件的图片文件"
                return batch_result
            ...
            for img_file in image_files:
                ...
                if result["success"]:
                    ...
                    logger.info(f"处理成功: {img_file.name}")
                else:
                    batch_result["failed_files"] += 1
                    logger.error(f"处理失败: {img_file.name}")
                
                batch_result["files"].append({
                    "file": img_file.name,
                    "success": result["success"],
                    "message": result.get("message", ""),
                    "new_size": result.get("new_size", (0, 0))
                })
            
            batch_result.update({
                "success": True,
                "message": f"批量处理完成: 成功 {batch_result['processed_files']}/{batch_result['total_files']}"
            })
            
            return batch_result
            
        except Exception as e:
            logger.error(f"批量缩放失败: {e}")
            batch_result["message"] = f"批量缩放失败: {e}"
            return batch_result
